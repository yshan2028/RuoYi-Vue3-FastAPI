...
@Version        :  V1.0.0
------------------------------------
@File           :  module_admin/service/dept_service.py
@Description    :  
...
class DeptService:
    ...
    def list_to_tree(cls, permission_list: list) -> list:
        ...
        """
        permission_list = [
            dict(id=int(item.dept_id), label=item.dept_name, value=str(item.dept_id), parent_id=str(item.parent_id))
            for item in permission_list
        ]
        # 转成id为key的字典
        mapping: dict = {str(i["id"]): i for i in permission_list}
